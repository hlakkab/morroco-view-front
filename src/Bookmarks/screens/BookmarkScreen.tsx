...
const BookmarkScreenContent: React.FC = () => {
  ...
  const { bookmarks, loading, error, currentPage, totalPages, totalElements, pageSize } = useAppSelector((state) => state.bookmark);
  ...
  useEffect(() => {
    dispatch(fetchBookmarks({ page: 0, size: pageSize }));
  }, [dispatch]);

  // Handle page change
  const handlePageChange = (page: number) => {
    // Convert to 0-based index for the API
    dispatch(fetchBookmarks({ page: page - 1, size: pageSize }));
  };
